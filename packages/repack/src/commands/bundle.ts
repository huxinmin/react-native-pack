...
import { getWebpackConfigOutterPath } from './utils/getWebpackConfigOutterPath';
...
export function bundle(_: string[], config: Config, args: BundleArguments) {
  const webpackConfigPath = getWebpackConfigOutterPath(
    config.root,
    args.webpackConfig
  );
  ...
  const compiler = webpack(getWebpackConfig(webpackConfigPath));
