...
const mode = ReactNativeWebpack.getMode({ fallback: 'development' });
const dev = mode === 'development';
const context = ReactNativeWebpack.getContext();
const entry = ReactNativeWebpack.getEntry();
const platform = ReactNativeWebpack.getPlatform({
  fallback: process.env.PLATFORM,
});
const minimize = ReactNativeWebpack.isMinimizeEnabled({ fallback: !dev });
const devServer = ReactNativeWebpack.getDevServerOptions();
const reactNativePath = ReactNativeWebpack.getReactNativePath();
...
module.exports = {
   ...
   */
  entry: [
    ...ReactNativeWebpack.getInitializationEntries(reactNativePath, {
      hmr: devServer.hmr,
    }),
    entry,
  ],
  resolve: {
     ...
     */
    ...ReactNativeWebpack.getResolveOptions(platform),
   ...
   * `ReactNativeWebpack.OutputPlugin`, which is configured by default.
   */
  output: {
    clean: true,
  ...
  module: {
    ...
    rules: [
       ...
       * ```
       * ReactNativeWebpack.ASSET_EXTENSIONS.filter((ext) => ext !== 'svg')
       * ```
       */
      {
        test: ReactNativeWebpack.getAssetExtensionsRegExp(
          ReactNativeWebpack.ASSET_EXTENSIONS
        ),
        use: {
          loader: 'rn-fast-pack/assets-loader',
          options: {
            platform,
             ...
             */
            scalableAssetExtensions: ReactNativeWebpack.SCALABLE_ASSETS,
          },
        },
      },
    ],
  ...
  plugins: [
     ...
     */
    new ReactNativeWebpack.AssetsResolverPlugin({
      platform,
    }),
     ...
     */
    new ReactNativeWebpack.TargetPlugin(),
     ...
     */
    new ReactNativeWebpack.OutputPlugin({
      platform,
      devServerEnabled: devServer.enabled,
     ...
     */
    new ReactNativeWebpack.DevServerPlugin({
      platform,
      ...devServer,
    }),
     ...
     */
    new ReactNativeWebpack.LoggerPlugin({
      platform,
      devServerEnabled: devServer.enabled,
      output: {
        console: true,
